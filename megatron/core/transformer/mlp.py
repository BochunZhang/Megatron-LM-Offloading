...
class MLP(MegatronModule):
    ...
    def forward(self, hidden_states, per_token_scale=None, **kwargs):
        ...
        # [s, b, 4 * h/p]
        nvtx_range_push(suffix="mlp.linear_fc1")
        intermediate_parallel, bias_parallel = self.linear_fc1(hidden_states)
        nvtx_range_pop(suffix="mlp.linear_fc1")

        nvtx_range_push(suffix="mlp.activation")
        if self.config.use_te_activation_func:
            ...
            if per_token_scale is not None:
                ...
                intermediate_parallel = intermediate_parallel * per_token_scale.unsqueeze(-1)
                intermediate_parallel = intermediate_parallel.to(original_dtype)
        nvtx_range_pop(suffix="mlp.activation")

        # [s, b, h]
        nvtx_range_push(suffix="mlp.linear_fc2")
        output, output_bias = self.linear_fc2(intermediate_parallel)
        nvtx_range_pop(suffix="mlp.linear_fc2")
    ...
    #         bias_act_output = bias_act_func(fc1_output, bias_parallel)
    #     nvtx_range_pop(suffix="mlp.activation")
    ...
    #         output, name="linear_fc2", forced_released_tensors=[]
    #     )
    #     nvtx_range_pop(suffix="mlp.linear_fc2")
