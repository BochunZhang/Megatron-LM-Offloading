...
def set_nvtx_name(module: torch.nn.Module, name: str):
    from megatron.core.transformer.transformer_layer import TransformerLayer

    def _nvtx_name(module: torch.nn.Module, prefix: str):
        if isinstance(module, TransformerLayer):
            if module.is_moe_layer:
                module.nvtx_name = prefix.replace("layers", "moe.layer")
            else:
                module.nvtx_name = prefix.replace("layers", "dense.layer")
        module.nvtx_name = prefix
        for name, module in module.named_children():
            _nvtx_name(module, f"{prefix}.{name}")

    _nvtx_name(module, name)