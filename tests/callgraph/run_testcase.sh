...
usage() {
    ...
    echo "                          - Test with dp=ep=4, alltoall dispatcher, seq_len=4096,"
    ...
    echo "                          - Test with dp=ep=4, hybridep dispatcher, seq_len=4096,"
    echo "                            mbs=1/2/4/8, 2 dense layers + 3 MoE layers, 32 experts, cuda graph enabled"
    echo "  measure-operators-hybridep-offload-4gpu"
    echo "  measure-pp-vpp-4gpu    "
    echo "                          - Test with pp=4, vpp=4, 4 GPUs, seq_len=4096,"
    echo "                            mbs=1, gbs=64, 3 dense layers + 26 MoE layers, 256 experts,"
    echo "                            one test only"
    echo ""
    echo "Example:"
    echo "  $0 --test measure-operators-4gpu"
    echo "  $0 --test measure-operators-with-graph-4gpu"
    echo "  $0 --test measure-operators-hybridep-4gpu"
    echo "  $0 --test measure-operators-hybridep-offload-4gpu"
