#!/bin/bash
# bin/train.sh - Layer 1: Execution Engine
# Responsibility: Execute training, no testcase logic, only accepts low-level parameters
...
while true; do
    case "$1" in
        ...
        --enable-cuda-graph) ENABLE_CUDA_GRAPH=true; shift ;;
        --profile) ENABLE_PROFILE=true; shift ;;
        --activation-offload) OFFLOAD_ACTIVATION=true; CPU_OFFLOADING=true; CPU_OFFLOADING_DOUBLE_BUFFERING=true; shift ;;
        --weights-offload) OFFLOAD_WEIGHTS=true; CPU_OFFLOADING=true; CPU_OFFLOADING_DOUBLE_BUFFERING=true; shift ;;
...
# Validate dispatcher and check/install deep_ep if needed
case "$DISPATCHER" in
    deepep|hybridep)
        check_and_install_deep_ep "$DISPATCHER" || {
...
echo "  Dispatcher=$DISPATCHER, CUDA_Graph=$ENABLE_CUDA_GRAPH"
echo "  Profile=$ENABLE_PROFILE"
echo "  Offload: activation=$OFFLOAD_ACTIVATION, weights=$OFFLOAD_WEIGHTS, optimizer=$OPTIMIZER_OFFLOAD"
